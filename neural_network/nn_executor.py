...
def get_probability_score(nn_model, feature_vector):
    ...
    score = nn_model.predict(feature_vector)
    score = score[0][0]
    return score
...
if __name__ == "__main__":
    ...
    trained_model_path = "../result/nn_model.h5"
    result_output = "../result/NN.txt"
    vector_dimension = 50
    # load model
    nn_model = load_model(trained_model_path)
    # summarize model.
    nn_model.summary()
    # run the model for the validation dataset
    run_validation(validation_file_path, w2vec_pre_trained_model, nn_model, result_output, .50,
                   vector_dimension)
    sys.exit()
